...
def searchpoint_cytomine(repository, current, clf, mx, my, cm, depths, window_size, image_type, npred, feature_type,
                         feature_parameters):
...
if __name__ == "__main__":
    ...
    p.add_option('--model_names', type="string", dest="cytomine_model_names",
                 help="The names of the models to use for detection (separated by commas, no spaces)")
    ...
    for model in model_names:
        ...
        for id_term in term_list:
            ...
            for j in pr_im:
                (x, y, y_o) = searchpoint_cytomine(repository, j, clf, mx, my, cm,
                                                   1. / (2. ** np.arange(int(par['model_depth']))),
                                                   int(par['window_size']), image_type, int(par['model_npred']),
                                                   par['feature_type'], fparams)
                circle = Point(x, y)
                location = circle.wkt
                new_annotation = cytomine_connection.add_annotation(location, j)
                cytomine_connection.add_annotation_term(new_annotation.id, term=id_term, expected_term=id_term,
                                                        rate=1.0, annotation_term_model=models.AlgoAnnotationTerm)
                if j in idim_to_i:
