...
def main():
    ...
    p.add_option('--model_name', type="string", dest="cytomine_model_name",
    ...
    cytomine_connection.update_job_status(job, status=job.RUNNING, progress=0,
    ...
    for id_img in pr_im:
        ...
        np.savez_compressed('%spmap_phase1-%s-%d.npy' % (repository, options.cytomine_model_name, id_img),
                            probability_map)
    ...
    for id_term in id_terms:
        reg = joblib.load(
            '%s%s_dmbl_regressor_phase2_%d.pkl' % (parameters['model_load_from'], options.cytomine_model_name, id_term))
        ...
        for id_img in pr_im:
            probability_map = \
            np.load('%spmap_phase1-%s-%d.npy.npz' % (repository, options.cytomine_model_name, id_img))['arr_0']
            probability_map_phase_2 = agregation_phase_2(repository, id_img, i, probability_map, reg,
                                                         float(par['model_delta']), feature_offsets_2,
                                                         int(par['model_filter_size']), float(par['model_beta']),
                                                         int(par['model_n_iterations']))
            np.savez_compressed(
                '%spmap_phase2-%s-%d-%d.npy' % (repository, options.cytomine_model_name, id_img, id_term),
                probability_map_phase_2)
            i += 1
    ...
    for id_img in pr_im:
        probability_map_phase_2 = \
        np.load('%spmap_phase2-%s-%d-%d.npy.npz' % (repository, options.cytomine_model_name, id_img, id_terms[0]))[
            'arr_0']
        ...
        for id_term in id_terms[1:]:
            pmap[:, :, i] = \
            np.load('%spmap_phase2-%s-%d-%d.npy.npz' % (repository, options.cytomine_model_name, id_img, id_term))[
                'arr_0']
            i += 1
        x_final, y_final = compute_final_solution_phase_3(xc, yc, pmap, int(par['model_ncandidates']),
                                                          float(par['model_sde']), float(par['model_delta']),
                                                          int(par['model_T']), edges)
        i = 0
        for id_term in id_terms:
