    ...
    if feature_type == 'raw':
        ...
        for resolution in depths:
            ...
            for i in range(n_sq):
                ...
                dataset[:, feature_index] = zero + (-2 * un) + deux + (-2 * trois) + (4 * quatre) + (
                            -2 * cinq) + six + (-2 * sept) + huit
                feature_index += 1
        return dataset
...
def dataset_image_rot(repository, Xc, Yc, R, RMAX, proportion, step, i, ang, feature_type, feature_parameters, depths,
                      window_size, image_type):
    x_v = []
    y_v = []
    REP = []
    IMGS = []
    ...
    (x_r, y_r, im) = rotate_coordinates(repository, i, np.round(np.array(x_v)), np.round(np.array(y_v)), ang,
                                        image_type)
    (hr, wr) = im.shape
    hr -= 1
    wr -= 1
...
def build_datasets_rot_mp(repository, imc, Xc, Yc, R, RMAX, proportion, step, ang, window_size, feature_type,
                          feature_parameters, depths, nimages, image_type, njobs):
    job_pool = Pool(njobs)
    job_args = [(repository, Xc[i], Yc[i], R, RMAX, proportion, step, imc[i], (np.random.ranf() * 2 * ang) - ang,
                 feature_type, feature_parameters, depths, window_size, image_type) for i in range(nimages)]
...
def main():
    opt_parser = optparse.OptionParser(description='Cytomine Landmark Detection : Model building',
                                       prog='Cytomine Landmark Detector : Model builder', version='0.1')
    opt_parser.add_option('--cytomine_host', type="string", default='beta.cytomine.be', dest="cytomine_host",
                          help="The Cytomine host (eg: beta.cytomine.be, localhost:8080)")
    opt_parser.add_option('--cytomine_public_key', type="string", default='XXX', dest="cytomine_public_key",
                          help="Cytomine public key")
    opt_parser.add_option('--cytomine_private_key', type="string", default='YYY', dest="cytomine_private_key",
                          help="Cytomine private key")
    opt_parser.add_option('--cytomine_id_software', type="int", dest="cytomine_id_software",
                          help="The Cytomine software identifier")
    opt_parser.add_option('--cytomine_base_path', type="string", default='/api/', dest="cytomine_base_path",
                          help="Cytomine base path")
    opt_parser.add_option('--cytomine_working_path', default="/tmp/", type="string", dest="cytomine_working_path",
                          help="The working directory (eg: /tmp)")
    opt_parser.add_option('--cytomine_id_project', type="int", dest="cytomine_id_project",
                          help="The Cytomine project identifier")
    opt_parser.add_option('--cytomine_id_terms', type='string', dest='cytomine_id_terms',
    ...
    opt_parser.add_option('--cytomine_training_images', default='all', type='string', dest='cytomine_training_images',
    ...
    opt_parser.add_option('--image_type', type='string', default='jpg', dest='image_type',
                          help="The type of the images that will be used (jpg, bmp, png,...)")
    opt_parser.add_option('--model_njobs', type='int', default=1, dest='model_njobs',
                          help="The number of processors used for model building")
    opt_parser.add_option('--model_R', type='int', default=6, dest='model_R',
                          help="Max distance for extracting landmarks")
    opt_parser.add_option('--model_RMAX', type='int', default=200, dest='model_RMAX',
    ...
    opt_parser.add_option('--model_npred', type='int', default=50000, dest='model_npred',
    ...
    opt_parser.add_option('--model_ntimes', type='int', default=3, dest='model_ntimes',
    ...
    opt_parser.add_option('--model_depth', type='int', default=5, dest='model_depth',
                          help="Number of resolutions to use")
    opt_parser.add_option('--model_step', type='int', default=1, dest='model_step',
    ...
    opt_parser.add_option('--model_feature_type', type='string', default='haar', dest='model_feature_type',
                          help='The type of feature (raw, sub, haar or gaussian).')
    opt_parser.add_option('--model_feature_haar_n', type='int', default=1600, dest='model_feature_haar_n',
                          help='Haar-Like features only. Number of descriptors for a pixel. Must be a multiple of 5*depths.')
    opt_parser.add_option('--model_feature_gaussian_n', type='int', default=1600, dest='model_feature_gaussian_n',
                          help='Gaussian features only. Number of descriptors for a pixel. Must be a multiple of depths.')
    opt_parser.add_option('--model_feature_gaussian_std', type='float', default=20., dest='model_feature_gaussian_std',
                          help='Gaussian features only. Standard deviation for the gaussian.')
    opt_parser.add_option('--model_save_to', type='string', default='/tmp/', dest='model_save_to',
    ...
    opt_parser.add_option('--verbose', type="string", default="0", dest="verbose",
                          help="Turn on (1) or off (0) verbose mode")
    opts, argus = opt_parser.parse_args(args=sys.argv)
    conn = cytomine.Cytomine(opts.cytomine_host, opts.cytomine_public_key, opts.cytomine_private_key,
                             base_path=opts.cytomine_base_path, working_path=opts.cytomine_working_path,
                             verbose=str2bool(opts.verbose))
    ...
    job_parameters = {'cytomine_id_terms': opts.cytomine_id_terms,
                      'cytomine_training_images': opts.cytomine_training_images, 'model_R': opts.model_R,
                      'model_njobs': opts.model_njobs, 'model_RMAX': opts.model_RMAX, 'model_P': opts.model_P,
                      'model_npred': opts.model_npred, 'model_ntimes': opts.model_ntimes,
                      'model_angle': opts.model_angle, 'model_depth': opts.model_depth, 'model_wsize': opts.model_wsize,
                      'model_ntrees': opts.model_ntrees, 'model_step': opts.model_step,
                      'forest_max_features': ((2 * opts.model_wsize) ** 2) * opts.model_depth,
                      'forest_min_samples_split': 2, 'model_name': opts.model_name,
                      'model_feature_type': opts.model_feature_type, 'model_feature_haar_n': opts.model_feature_haar_n,
                      'model_feature_gaussian_n': opts.model_feature_gaussian_n,
                      'model_feature_gaussian_std': opts.model_feature_gaussian_std}
    ...
    for id_term in term_list:
        ...
        if opts.model_feature_type.lower() == 'gaussian':
            ...
            feature_parameters = np.round(
                np.random.multivariate_normal([0, 0], std_matrix, opts.model_feature_gaussian_n)).astype(int)
        ...
        for times in range(opts.model_ntimes):
            ...
            T = build_datasets_rot_mp(repository, tr_im, xc, yc, opts.model_R, opts.model_RMAX, opts.model_P,
                                      opts.model_step, rangrange, opts.model_wsize, opts.model_feature_type,
                                      feature_parameters, depths, nimages, opts.image_type, opts.model_njobs)
