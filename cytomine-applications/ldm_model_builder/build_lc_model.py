...
def build_dataset_image_offset_mp(repository, xc, yc, ims, dmax, nsamples, h2, v2, h3, v3, sq, n_jobs):
    nimages = xc.size
    jobargs = [(repository, ims[image_number], xc[image_number], yc[image_number], dmax, nsamples, h2, v2, h3, v3, sq)
...
def main():
    ...
    p.add_option('--cytomine_training_images', type='string', default='all', dest='cytomine_training_images',
                 help="IDs of the training images. Must be separated by commas, no spaces. 'all' takes all the available annotated images.")
    ...
    p.add_option('--model_njobs', type='int', default=4, dest='model_njobs',
    ...
    p.add_option('--model_n_samples', type='int', default=200, dest='model_n_samples',
    ...
    p.add_option('--model_n_reduc', type='int', default=2, dest='model_n_reduc',
    ...
    p.add_option('--model_alpha', type='float', default=0.5, dest='model_alpha',
    ...
    cytomine_connection = cytomine.Cytomine(options.cytomine_host, options.cytomine_public_key,
                                            options.cytomine_private_key, base_path=options.cytomine_base_path,
                                            working_path=options.cytomine_working_path, verbose=options.verbose)
    ...
    for id_term in term_list:
        (dataset, rep, img) = build_dataset_image_offset_mp(repository, Xc[:, term_to_i[id_term]],
                                                            Yc[:, term_to_i[id_term]], im_list, options.model_D_MAX,
                                                            options.model_n_samples, h2, v2, h3, v3, sq,
                                                            options.model_njobs)
