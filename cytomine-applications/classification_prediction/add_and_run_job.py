...
def main(argv):
    ...
    p.add_option('-i', '--cytomine_id_image', type="int", dest="cytomine_id_image",
    ...
    p.add_option('--cytomine_id_userjob', type="int", dest="cytomine_id_userjob",
                 help="The Cytomine user (job) id of annotations to classify with the model")
    ...
    for k, annotation in enumerate(candidate_annotations.data()):
        ...
        conn.add_annotation_term(new_annotation.id, int(y_predict[j]), int(y_predict[j]), y_rate[j],
