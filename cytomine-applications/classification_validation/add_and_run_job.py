...
# id of projets from which we dump annotations to build the training dataset
...
def main(argv):
    ...
    p.add_option('--cytomine_fixed_tile', type="string", default="False", dest="cytomine_fixed_tile",
                 help="Force fixed tile size crop around annotations")
    p.add_option('--cytomine_n_shifts', type='int', dest='cytomine_n_shifts',
                 help="number of translated (shifted) crops extracted for each annotation")
    ...
    p.add_option('--cytomine_excluded_terms', type='string', default='5735', dest='cytomine_excluded_terms',
    ...
    if run_by_user_job == False:
        parameters_values = conn.add_job_parameters(user_job.job, conn.get_software(parameters['cytomine_id_software']),
                                                    pyxit_parameters)
    ...
    for annotation in annotations.data():
        ...
        for term in annotation.term:
            ...
            if hasattr(annotation, "filename") and (annotation.filename in predict):
                ...
                conn.add_annotation_term(annotation.id, p[0], term, annotation_term.rate,
