...
def main(argv):
    ...
    p.add_option('--cytomine_max_image_size', type='int', dest='cytomine_max_image_size',
                 help="max_image_size to apply detection")
    p.add_option('--cytomine_erode_iterations', type='int', dest='cytomine_erode_iterations', help="number of erosions")
    p.add_option('--cytomine_athreshold_constant', type='int', dest='cytomine_athreshold_constant',
                 help="increase/decrease value of threshold")
    p.add_option('--cytomine_athreshold_blocksize', type='int', dest='cytomine_athreshold_blocksize',
                 help="size of neighboordhood to compute threshold")
    p.add_option('--cytomine_dilate_iterations', type='int', dest='cytomine_dilate_iterations',
                 help="number of dilatations")
    ...
    images = conn.dump_project_images(id_project=idProject, dest_path="images", override=True,
                                      max_size=parameters['cytomine_max_image_size'])
    ...
    job_parameters_values = conn.add_job_parameters(user_job.job, conn.get_software(parameters['cytomine_id_software']),
                                                    parameters)

    for obj in images:
        if True:
            print "Processing Cytomine imageid %d jobid %d" % (obj.id, job.userJob)
            job = conn.update_job_status(job, status=job.RUNNING, progress=i / len(images) * 100,
                                         status_comment="Processing images...")
