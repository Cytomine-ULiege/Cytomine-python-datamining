...
parameters = {
    ...
    'cytomine_annotation_projects': [669418],
# to be adapted: id of projets from which we dump annotations for learning
...
def main(argv):
    ...
    job = conn.update_job_status(job, status=job.RUNNING, status_comment="[pyxit.main] Initializing PyxitClassifier...",
                                 progress=25)
    ...
    job = conn.update_job_status(job, status_comment="[pyxit.main] Extracting %d subwindows from each image in %s" % (
    pyxit_parameters['pyxit_n_subwindows'], pyxit_parameters['dir_ls']), progress=50)
    time.sleep(3)
