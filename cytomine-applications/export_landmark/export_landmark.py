...
def main(argv):
    ...
    parser.add_option('--cytomine_id_software', type="int", dest="cytomine_id_software",
                      help="The Cytomine software identifier")
    parser.add_option('--cytomine_id_project', type="int", dest="cytomine_id_project",
    ...
    for image in images:
        ...
        progress += delta
        job = cytomine_connection.update_job_status(job, status=job.RUNNING, status_comment="Fetching data",
                                                    progress=progress)

        annotations = cytomine_connection.get_annotations(id_project=parameters['cytomine_id_project'], showWKT=True,
                                                          id_image=image.id, reviewed_only=True)
    ...
    job_data = cytomine_connection.add_job_data(job, key=job.id, filename='%s%s.csv' % (
    parameters['cytomine_working_path'], str(parameters['cytomine_id_project'])))

    job = cytomine_connection.update_job_status(job, status=job.TERMINATED, status_comment="File available",
                                                progress=100)
