...
def _parallel_confidence_map(pixels, _Y, offset, boxes, tile_width, tile_height, n_classes, pyxit_target_width,
                             pyxit_target_height):
...
def rasterize_tile_roi_union(nx, ny, points, local_tile_component, roi_annotations_union, whole_slide, reader):
    tile_component = \
    whole_slide.convert_to_real_coordinates(whole_slide, [local_tile_component], reader.window_position, reader.zoom)[0]
    ...
    local_tile_roi_union_components = whole_slide.convert_to_local_coordinates(whole_slide, tile_roi_union_components,
                                                                               reader.window_position, reader.zoom)
    local_tile_roi_union_polygons = [shapely.geometry.Polygon(component[0], component[1]) for component in
                                     local_tile_roi_union_components]
...
def main(argv):
    ...
    p.add_option('--cytomine_union', type="string", default="0", dest="cytomine_union",
                 help="Turn on union of geometries")
    # p.add_option('--cytomine_postproc', action="store_true", default=False, dest="cytomine_postproc", help="Turn on postprocessing")
    p.add_option('--cytomine_postproc', type="string", default="0", dest="cytomine_postproc",
                 help="Turn on postprocessing")
    ...
    p.add_option('--cytomine_min_size', type="int", default=0, dest="cytomine_min_size",
                 help="minimum size (area) of annotations")
    p.add_option('--cytomine_max_size', type="int", default=10000000000, dest="cytomine_max_size",
                 help="maximum size (area) of annotations")
    ...
    p.add_option('--cytomine_mask_internal_holes', type='string', default="0", dest="cytomine_mask_internal_holes",
                 help="Turn on precise hole finding")

    p.add_option('-i', '--cytomine_id_image', type='int', dest='cytomine_id_image', help="image id from cytomine",
                 metavar='IMAGE')
    ...
    p.add_option('--cytomine_tile_min_stddev', type='int', default=5, dest='cytomine_tile_min_stddev',
                 help="tile minimum standard deviation")
    p.add_option('--cytomine_tile_max_mean', type='int', default=250, dest='cytomine_tile_max_mean',
                 help="tile maximum mean")
    p.add_option('--cytomine_union_min_length', type='int', default=5, dest='cytomine_union_min_length', help="union")
    p.add_option('--cytomine_union_bufferoverlap', type='int', default=5, dest='cytomine_union_bufferoverlap',
                 help="union")
    p.add_option('--cytomine_union_area', type='int', default=5, dest='cytomine_union_area', help="union")
    p.add_option('--cytomine_union_min_point_for_simplify', type='int', default=5,
                 dest='cytomine_union_min_point_for_simplify', help="union")
    ...
    p.add_option('--cytomine_union_nb_zones_width', type='int', default=5, dest='cytomine_union_nb_zones_width',
                 help="union")
    p.add_option('--cytomine_union_nb_zones_height', type='int', default=5, dest='cytomine_union_nb_zones_height',
                 help="union")
    ...
    p.add_option('--cytomine_predict_term', type='int', dest='cytomine_predict_term',
                 help="term id of predicted term (binary mode)")
    p.add_option('--cytomine_roi_term', type='string', dest='cytomine_roi_term',
                 help="term id of region of interest where to count)")
    # p.add_option('--cytomine_reviewed_roi', action="store_true", default=False, dest="cytomine_reviewed_roi", help="Use reviewed roi only")
    p.add_option('--cytomine_reviewed_roi', type='string', default="0", dest="cytomine_reviewed_roi",
                 help="Use reviewed roi only")
    ...
    p.add_option('--cytomine_predict_step', type='int', dest='cytomine_predict_step',
                 help="pyxit step between successive subwindows")
    p.add_option('--pyxit_save_to', type='string', dest='pyxit_save_to',
                 help="pyxit segmentation model file")  # future: get it from server db
    ...
    p.add_option('--pyxit_post_classification', type="string", default="0", dest="pyxit_post_classification",
                 help="pyxit post classification of candidate annotations")

    p.add_option('--pyxit_post_classification_save_to', type='string', dest='pyxit_post_classification_save_to',
                 help="pyxit post classification model file")  # future: get it from server db
    p.add_option('--pyxit_colorspace', type='int', dest='pyxit_colorspace',
                 help="pyxit colorspace encoding")  # future: get it from server db
    p.add_option('--pyxit_nb_jobs', type='int', dest='pyxit_nb_jobs',
    ...
    image_instances = conn.get_project_image_instances(parameters['cytomine_id_project'])
    ...
    for image in images:
        ...
        while True:
            ...
            if np.count_nonzero(roi_mask):
                ...
                if (((tilestddev[0] < mindev) and (tilestddev[1] < mindev) and (tilestddev[2] < mindev)) or (
                    ...
                    image_filename = "%s/slides/project-%d/tiles/%d-zoom_%d-tile_%d_x%d_y%d_w%d_h%d.png" % (
                    parameters['cytomine_working_path'], parameters["cytomine_id_project"], id_image, zoom, wsi, posx,
                    posy, poswidth, posheight)
                    ...
                    outputfilename = "%s/slides/project-%d/tiles/%d-zoom_%d-tile_%d_xxOUTPUT-%dx%d.png" % (
                    parameters["cytomine_working_path"], parameters["cytomine_id_project"], id_image, zoom, wsi,
                    ...
                    if parameters['cytomine_mask_internal_holes']:
                        ...
                        components = whole_slide.convert_to_real_coordinates(whole_slide, components,
                                                                             reader.window_position, reader.zoom)
                        geometries.extend(get_geometries(components))
                    else:
            ...
            while (not retrieved):
                try:
                    annotations = conn.get_annotations(id_user=job.userJob, id_image=id_image,
                                                       id_project=parameters['cytomine_id_project'], showGIS=True)
                    retrieved = True
                except socket.timeout, socket.error:
        ...
        if parameters['cytomine_union']:
            print "TIME : %s" % time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            print "Union of polygons for job %d and image %d, term: %d" % (
            job.userJob, id_image, parameters['cytomine_predict_term'])
            start = time.time()
        ...
        if parameters['pyxit_post_classification']:
            ...
            candidate_annotations = conn2.get_annotations(id_user=job.userJob, id_image=id_image,
                                                          id_term=parameters['cytomine_predict_term'], showWKT=True,
                                                          showMeta=True)
            ...
            for k, annotation in enumerate(candidate_annotations.data()):
                ...
                if int(y_predict[j]) == parameters['cytomine_predict_term']:
                    ...
                    annotation.id, int(y_predict[j]), y_rate[j])
                new_annotation = conn2.add_annotation(annotation.location, id_image)
                conn2.add_annotation_term(new_annotation.id, int(y_predict[j]), int(y_predict[j]), y_rate[j],
                                        annotation_term_model=models.AlgoAnnotationTerm)
